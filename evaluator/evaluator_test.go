...
func TestArrayLiterals(t *testing.T) {
	...
	ed := testEval(input)
	result, ok := ed.(*object.Array)
	if !ok {
		t.Fatalf("object is not Array. got=%T (%+v)", ed, ed)
	}
...
func TestArrayIndexLiterals(t *testing.T) {
	...
	for _, tt := range tests {
		...
		integer, ok := tt.expected.(int)
		if ok {
			testIntegerObject(t, ed, int64(integer))
		} else {
			testNullObject(t, ed)
		}
	}
...
func TestHashLiterals(t *testing.T) {
	...
	ed := testEval(input)
	result, ok := ed.(*object.Hash)
	if !ok {
		t.Fatalf("object is not Hash. got=%T (%+v)", ed, ed)
	}
...
func TstDefineMacros(t *testing.T) {
	input := `
	let number = 1;
	...
	if len(macro.Parameters) != 2 {
		t.Fatalf("Wrong number of macro paramerers. got=%d", len(macro.Parameters))
	}
